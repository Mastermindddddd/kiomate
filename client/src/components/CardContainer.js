...
const CardContainer = () => {
  ...
  const getUser = async () => {
    try {
      const response = await axios.get('https://kiomate.vercel.app//user', {
        params: { userId }
      });
  ...
  const getGenderedUsers = async () => {
    try {
      const response = await axios.get('https://kiomate.vercel.app//gendered-users', {
  ...
  const updateMatches = async (matchedUserId) => {
    try {
      await axios.put('https://kiomate.vercel.app//addmatch', {
        userId,
        matchedUserId
      });
      getUser();
