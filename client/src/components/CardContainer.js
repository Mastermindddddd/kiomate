...
const CardContainer = () => {
  ...
  const getUser = async () => {
    try {
      const response = await axios.get('https://kiomate-mastermindddddds-projects.vercel.app/user', {
        params: { userId }
      });
  ...
  const updateMatches = async (matchedUserId) => {
    try {
      await axios.put('https://kiomate-mastermindddddds-projects.vercel.app/addmatch', {
        userId,
        matchedUserId
      });
      getUser();
